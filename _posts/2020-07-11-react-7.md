...
위에서 살펴본 것들은 클래스형 컴포넌트에서 ref를 사용하는 방법이었습니다.
...
const TestComponent = (props) => {
    ...
    const onClick = function () {
        domtoimage.toPng(personInfo.current) // 위에서 useRef()로 만든 객체에 .current로 접근
        .then(function (blob) {
...
이후 `personInfo.current`를 하면, 위에서 부여한 div 요소에 접근 가능한 id처럼 사용할 수 있습니다.

(`personInfo.current`는 이 div 요소를 가리킵니다.)
