...
쉽게 말해서 우리가 짜고 있는 react 코드에서 무언가 메인 프로세스에서만 할 수 있는 일(메뉴 표시, 다이얼로그, 윈도우 관련, Node.js API 기능 등)이 필요할 경우, react 등의 기존 자바스크립트 코드에서는 접근할 수 없는 일이기 때문에, **프로세스 간 통신(IPC, Inter-Process Communication)으로 메인 프로세스에게 요청을** 해야 하기 때문이다.
